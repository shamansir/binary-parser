...
Parser.prototype.logTo = function(ctx) {
    ctx.pushCode('console.log((offset || 0).toString(16), "{0}")', this.toString());
};
...
Parser.prototype.toString = function() {
    ...
    if (this.type) {
        ...
        if ((this.type === 'Skip') || (this.type === 'Array') || (this.type === 'String')) {
            result.push('length:' + this.options.length);
        } else if (this.type === 'Choice') {
            result.push('tag:' + this.options.tag);
        }
    }
    if (this.options.type) result.push('{' + this.options.type + '}');
    if (this.varName) result.push('varName:' + this.varName);
    if (this.alias) result.push('alias:' + this.alias);
...
Object.keys(PRIMITIVE_TYPES).forEach(function(type) {
    Parser.prototype['generate' + type] = function(ctx) {
        var v = ctx.generateVariable(this.varName);
        ctx.pushCode('{0} = buffer.read{1}(offset);', v, type);
        ctx.pushCode('offset += {0};', PRIMITIVE_TYPES[type]);
        ctx.pushCode('console.log({0})', v);
    };
});
...
Parser.prototype.generateString = function(ctx) {
    ...
    ctx.pushCode('console.log({0})', name);
};
...
Parser.prototype.generateArray = function(ctx) {
    ...
    ctx.pushCode('console.log({0})', lhs);
};
...
Parser.prototype.generateChoiceCase = function(ctx, varName, type) {
    if (typeof type === 'string') {
        if (!aliasRegistry[type]) {
            var v = ctx.generateVariable(this.varName);
            ctx.pushCode('{0} = buffer.read{1}(offset);', v, NAME_MAP[type]);
            ctx.pushCode('offset += {0};', PRIMITIVE_TYPES[NAME_MAP[type]]);
            ctx.pushCode('console.log({0}.result);', v);
        } else {
            var tempVar = ctx.generateTmpVariable();
            ctx.pushCode('var {0} = {1}(offset);', tempVar, FUNCTION_PREFIX + type);
            ctx.pushCode('{0} = {1}.result; offset = {1}.offset;', ctx.generateVariable(this.varName), tempVar);
            ctx.pushCode('console.log({0}.result);', tempVar);
            if (type !== this.alias) ctx.addReference(type);
        }
    } else if (type instanceof Parser) {
...
Parser.prototype.generateNest = function(ctx) {
    ...
    if (this.options.type instanceof Parser) {
        ...
        ctx.popPath();
        ctx.pushCode('console.log({0})', nestVar);
